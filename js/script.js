    ...
    if(mine_map[i][j] != -1){
      for( row = -1; row < 2; row++){
        for( column = -1; column < 2; column++){
          if( (i + row) < 0 || (i + row) > (N - 1) || (j + column) < 0 || (j + column) > (M - 1) ){
            continue;
...
document.getElementById('commentary').innerHTML = '縦' + N + '×横' + M + 'マス,地雷' + number_of_bumb + '個';
...
function troutClick(){
  ...
  var clickTroutId,troutIdArr,timerFlag = 0;
  for(let k = 0; k < trouts.length; k++){
    trouts[k].addEventListener('click', () => {
      ...
      clickTroutId = document.getElementById('troutId' + conversion(troutIdArr[0],troutIdArr[1]));
      i = troutIdArr[0];
      j = troutIdArr[1];
        ...
        if(mode == 'N'){
          if(mine_map[i][j] == -1){ //地雷だったら
            ...
            for(i1 = 0; i1 < N; i1++){
              for(j1 = 0; j1 < M; j1++){
                if(mine_map[i1][j1] == -1){
                  document.getElementById('troutId' + conversion(i1,j1)).classList.add('bomb');
                }else {
                  display_board[i1][j1] = 1;
                  if(mine_map[i1][j1] != 0){
                    document.getElementById('troutId' + conversion(i1,j1)).innerHTML = mine_map[i1][j1];
                  }
                  display(i1,j1);
                }
              }
            }
            document.getElementById('troutId' + conversion(i,j)).classList.add('explosion');
            display(i,j);
          }else if(mine_map[i][j] > 0){ //数字が入っていたら
            display_board[i][j] = 1; //開く
            display(i,j); //8近傍をmime_mapを画面上に表示
            document.getElementById('troutId' + conversion(i,j)).innerHTML = mine_map[i][j];
          }else{ //0だったら8近傍を開く
            display_board[i][j] = 1; //開く
            display(i,j); //表示
            open(i,j);
      ...
      if((count == ((N * M) - number_of_bumb)) && over != 1){
        ...
        //Twitterリンクの作成
        var clearTxt = '縦' + N + '×横' + M + 'マス,地雷' + number_of_bumb + '個のステージを' + time + 'でクリアしました！';
        var twHref = 'https://twitter.com/share?text=' + clearTxt + '&url=' + 'https://nose-free.com/minesweeper';
        var twLink = '<a href="' + twHref + '"><i class="fab fa-twitter"></i></a>';
        document.getElementById('twitter').innerHTML = twLink;
        document.getElementById('shareTxt').innerHTML = clearTxt + '<br><br>Twitterでシェアしてみよう！'
        //timerを止める
        stop();
      }
    }, false);
...
function open(x,y){
  ...
  for(row = -1; row < 2; row++){
    for(column = -1; column < 2; column++){
      // またははみ出している
      if((x + row) < 0 || (x + row) > (N - 1) || (y + column) < 0 || (y + column) > (M - 1) ){
        continue;
      }else if(mine_map[x + row][y + column] == -1){ //地雷
        continue;
      }else if(display_board[x + row][y + column] == 0){ //閉じている
        ...
        if(mine_map[x + row][y + column] == 0){
          ...
          document.getElementById('troutId' + conversion(x + row,y + column)).innerHTML = mine_map[x + row][y + column];
        }
      }
    }
  }
